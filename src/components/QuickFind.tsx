    ...
    <section className="relative z-20 -mt-24 px-6 mb-24">
      <div className="max-w-6xl mx-auto bg-white rounded-3xl shadow-2xl border border-neutral-100 overflow-hidden flex flex-col md:flex-row">
        ...
        <div className="w-full md:w-1/3 p-8 lg:p-12 bg-white border-b md:border-b-0 md:border-r border-neutral-100">
          <h3 className="text-sm font-bold text-neutral-400 tracking-widest uppercase mb-1">
            Find Your
          </h3>
          ...
          <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
            <div>
              ...
              <input
                ...
                value="Bihar"
                readOnly
                className="w-full border-b-2 border-neutral-200 py-2 text-neutral-900 font-bold focus:border-amber-500 outline-none bg-transparent cursor-default"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-neutral-500 mb-2 uppercase">
            ...
            {searchResult && (
              <div
                className={`p-4 rounded-xl border font-medium animate-in fade-in zoom-in-95 ${searchResult.count > 0 ? "bg-emerald-50 border-emerald-200 text-emerald-800" : "bg-rose-50 border-rose-200 text-rose-800"}`}
              >
                {searchResult.count > 0 ? (
                  <div>
                    ...
                    <button
                      ...
                      className="text-emerald-700 hover:text-emerald-900 font-bold text-sm underline underline-offset-2 transition-colors text-left"
                    >
                  ...
                  <div>
                    ...
                    <p className="text-rose-700/80 mb-4 text-xs leading-relaxed">
                      {searchResult.message ||
                        `We couldn't find available plots matching ${searchSqft} sqft or more right now. We often do custom groupings.`}
                    </p>
                    <button
                      type="button"
                      onClick={() => setIsEnquiryModalOpen(true)}
                      className="bg-rose-600 text-white px-4 py-2 rounded-lg font-bold text-xs hover:bg-rose-700 transition shadow-sm"
                    >
                      Contact Sales Team
                    </button>
                  </div>
            ...
            {!searchResult || searchResult.count > 0 ? (
              <button
                ...
                className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-4 rounded-xl mt-4 transition shadow-lg shadow-amber-600/20"
              >
                Search Plots
              </button>
            ) : null}
