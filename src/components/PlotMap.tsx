...
export default function PlotMap({
  ...
  const getPlotColor = (id: string) => {
    if (id.startsWith("A") || id.startsWith("RA"))
      return "bg-[#fca5a5] text-rose-900 border-rose-300";
    if (id.startsWith("B") || id.startsWith("RB"))
      return "bg-[#f9a8d4] text-pink-900 border-pink-300";
    if (id.startsWith("C") || id.startsWith("RC"))
      return "bg-[#7dd3fc] text-sky-900 border-sky-300";
    ...
    if (editMode) {
      if (e.shiftKey && lastSelectedId) {
        ...
        if (lastItem) {
          ...
          const sortedItemsToSelect = items
            .filter((i) => {
              return (
                i.x < maxX && i.x + i.w > minX && i.y < maxY && i.y + i.h > minY
              );
            })
            .sort((a, b) => {
              if (width > height) {
          ...
          setSelectedIds((prev) => {
            const prevWithoutNew = prev.filter(
              (id) => !sortedItemsToSelect.includes(id),
            );
      ...
      selectedIds.forEach((_, index) => {
        ...
        newItems = newItems.map((item) => {
          if (!selectedIds.includes(item.id) && item.id === targetId) {
            return {
              ...item,
              id: `${item.id}-old-${Math.floor(Math.random() * 1000)}`,
            };
          }
          return item;
        });
  ...
  const openBreakModal = () => {
    const itemsToBreak = items.filter(
      (i) => selectedIds.includes(i.id) && (i.w > 1 || i.h > 1),
    );
  ...
  const executeBreak = () => {
    setItems((prev) => {
      let newItems = [...prev];
      const itemsToBreak = newItems.filter(
        (i) => selectedIds.includes(i.id) && (i.w > 1 || i.h > 1),
      );
      ...
      itemsToBreak.forEach((item) => {
        ...
        const colBreaks = Array.from(
          { length: breakConfig.cols + 1 },
          (_, i) =>
            Math.round((item.x + (item.w * i) / breakConfig.cols) * 10000) /
            10000,
        );
        const rowBreaks = Array.from(
          { length: breakConfig.rows + 1 },
          (_, i) =>
            Math.round((item.y + (item.h * i) / breakConfig.rows) * 10000) /
            10000,
        );
  ...
  const exportJSON = () => {
    ...
    alert(
      "Map JSON copied to clipboard! You can paste it into layoutMatrix.json",
    );
  };
      ...
      <div className="w-full lg:w-80 shrink-0 flex flex-col gap-6 p-4 md:p-8 bg-neutral-50 rounded-3xl border border-neutral-100 relative z-10 shadow-sm order-2 lg:order-1">
          ...
          {editMode && (
            <>
              ...
              <div className="mt-4 text-xs text-neutral-500 font-medium leading-relaxed bg-neutral-100 p-3 rounded-lg border border-neutral-200">
                <strong>Tip:</strong> Click multiple items to select them
                sequentially. Clicking empty spaces creates new plots instantly.
                Click "Edit Selected" to bulk update IDs incrementally (e.g.
                A-101, A-102).
              </div>
            </>
          )}
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-bold text-sm text-neutral-500 tracking-wider uppercase">
              Map Controls
            </h3>
            <button
        ...
        <div>
          <h3 className="font-bold text-[10px] md:text-sm text-neutral-400 tracking-widest mb-4 uppercase">
            Plot Type & Area
          </h3>
            ...
            {[
              {
                ...
                area: "2700 SQ. FT.",
                dim: "45'-0\" X 60'-0\"",
                color: "bg-[#fca5a5] border-rose-300 text-rose-900",
              },
              {
                type: "B",
                area: "1800 SQ. FT.",
                dim: "36'-0\" X 50'-0\"",
                color: "bg-[#f9a8d4] border-pink-300 text-pink-900",
              },
              {
                type: "C",
                area: "1200 SQ. FT.",
                dim: "30'-0\" X 40'-0\"",
                color: "bg-[#7dd3fc] border-sky-300 text-sky-900",
              },
            ].map((item) => (
              <div
                key={item.type}
                className="flex items-center gap-3 bg-white p-2 md:p-3 rounded-2xl border border-neutral-100 shadow-sm"
              >
                <div
                  className={`w-10 h-10 md:w-12 md:h-12 rounded-xl border-2 flex items-center justify-center font-black text-lg md:text-xl shrink-0 ${item.color}`}
                >
                  {item.type}
                </div>
                <div>
            ...
            <>
              ...
              <div className="absolute top-6 left-6 z-40 pointer-events-none space-y-4">
                <div className="bg-white/90 backdrop-blur p-3 rounded-2xl shadow-xl border border-white/20 flex flex-col items-center">
                  ...
                  <span className="text-[10px] font-black mt-1 text-neutral-500 tracking-widest">
                    COMPASS
                  </span>
                </div>
              </div>
              ...
              {hoveredItem && hoveredItem.type === "road" && (
                <div className="absolute top-6 right-6 z-40 animate-in fade-in slide-in-from-right-4 duration-200 pointer-events-none">
                  <div className="bg-white/95 backdrop-blur-md p-5 rounded-2xl shadow-2xl border-l-[6px] border-emerald-600 shadow-emerald-900/20 flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-[10px] font-black text-emerald-600 tracking-widest uppercase mb-1">
                        Road Details
                      </div>
              ...
              >
                ...
                >
                  ...
                  {items.map((item) => {
                    const isSelected =
                      (!editMode && selectedPlot === item.id) ||
                      (editMode && selectedIds.includes(item.id));

                    let bgClass = "";
                    let borderClass =
                      "border-[0.5px] border-black/10 text-white";
                    ...
                    if (item.type === "road") {
                      ...
                      content = (
                        <div className="flex flex-col items-center justify-center">
                          ...
                          {item.label && (
                            <span className="text-[6px] md:text-[8px] uppercase font-black text-white/90 leading-tight text-center px-1">
                              {item.label}
                            </span>
                          )}
                        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95">
            ...
            <div className="p-6 space-y-4">
              {selectedIds.length === 1 ? (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBulkConfig({ ...bulkConfig, prefix: e.target.value })
                    }
                    className="w-full bg-neutral-50 border border-neutral-200 rounded-lg px-3 py-2 text-sm font-medium focus:ring-2 outline-none"
                  />
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 text-xs text-neutral-500 bg-blue-50 p-2 rounded border border-blue-100 text-blue-700">
                    <strong>Preview:</strong> {bulkConfig.prefix}
                    {bulkConfig.startNumber}, {bulkConfig.prefix}
                    {bulkConfig.startNumber + 1}, ...
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setBulkConfig({ ...bulkConfig, label: e.target.value })
                  }
                  className="w-full bg-neutral-50 border border-neutral-200 rounded-lg px-3 py-2 text-sm font-medium focus:ring-2 outline-none"
                />
              </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="w-10 h-10 rounded cursor-pointer border border-neutral-200 p-0"
                      />
                      <input
                        type="text"
                        placeholder="Leave blank for default"
                        value={bulkConfig.color || ""}
                        onChange={(e) =>
                          setBulkConfig({
                            ...bulkConfig,
                            color: e.target.value,
                        ...
                        className="flex-1 bg-neutral-50 border border-neutral-200 rounded-lg px-3 py-2 text-sm font-medium focus:ring-2 outline-none"
                      />
                    </div>
                  </div>
                </>
            ...
            <div className="p-4 bg-neutral-50 border-t border-neutral-200 flex items-center justify-between gap-3">
              <button
                ...
                className="px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors"
              >
                <Trash className="w-4 h-4" /> Delete
              </button>
              <button
                onClick={saveEditedItems}
                className="flex-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-4 py-2 text-sm font-bold flex items-center justify-center gap-2 transition-colors shadow-md"
              >
                <Save className="w-4 h-4" /> Save Items
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in-95">
            <div className="bg-neutral-100 p-4 border-b border-neutral-200 flex items-center justify-between">
              <h3 className="font-bold text-lg text-neutral-800 flex items-center gap-2">
                <Upload className="w-5 h-5 text-blue-600" /> Import Map Data
              </h3>
              <button
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-neutral-500">
                Paste your <strong>layoutMatrix.json</strong> content below to
                update the map layout instantly.
              </p>
              <textarea
                value={importText}
            ...
            <div className="p-4 bg-neutral-50 border-t border-neutral-200 flex items-center justify-end gap-3">
              <button
                onClick={() => setShowImportModal(false)}
                className="px-6 py-2 text-neutral-500 hover:text-black text-sm font-bold transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleImport}
                className="bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-8 py-2 text-sm font-bold flex items-center gap-2 transition-all shadow-md"
              >
                <Save className="w-4 h-4" /> Import Data
              </button>
            </div>
          </div>
        </div>
      ...
      {showBreakModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95">
            <div className="bg-neutral-100 p-4 border-b border-neutral-200 flex items-center justify-between">
              <h3 className="font-bold text-lg text-neutral-800 flex items-center gap-2">
                <Grid className="w-5 h-5 text-orange-500" /> Break Block
              </h3>
              <button
            ...
            <div className="p-6 space-y-4">
              <p className="text-xs text-neutral-500">
                Specify how many pieces to break the selected block(s) into.
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-neutral-500 uppercase mb-1">
            ...
            <div className="p-4 bg-neutral-50 border-t border-neutral-200 flex items-center justify-end gap-3">
              ...
              <button
                onClick={executeBreak}
                className="bg-orange-500 hover:bg-orange-600 text-white rounded-lg px-6 py-2 text-sm font-bold flex items-center gap-2 transition-all shadow-md"
              >
                <Grid className="w-4 h-4" /> Execute Break
              </button>
            </div>
          </div>
        </div>
